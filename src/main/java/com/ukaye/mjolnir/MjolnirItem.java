...
public class MjolnirItem  extends Item implements Vanishable {
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerEntity.isTouchingWaterOrRain()) {
                    if (!world.isClient) {
                        ...
                        if (j == 0) {
                            MjolnirEntity mjolnirEntity = new MjolnirEntity(world, playerEntity, stack);
                            mjolnirEntity.setProperties(playerEntity, playerEntity.pitch, playerEntity.yaw, 0.0F, 2.5F + (float) j * 0.5F, 1.0F);
                            if (playerEntity.abilities.creativeMode) {
                                mjolnirEntity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                            }

                            world.spawnEntity(mjolnirEntity);
                            world.playSoundFromEntity((PlayerEntity) null, mjolnirEntity, SoundEvents.ITEM_TRIDENT_THROW, SoundCategory.PLAYERS, 1.0F, 1.0F);
                            if (!playerEntity.abilities.creativeMode) {
